...
resource "auth0_client" "sample_client" {
  name = "Singpass AWS jwt.io client"
...
resource "auth0_client" "comp_app_client" {
  name = "Singpass AWS companion app"
  description = "singpass AWS companion app"
  app_type = "spa"
  oidc_conformant = true
  is_first_party = true
...
resource "auth0_connection" "singpass" {
  name = "Singpass-STG-Lambda"
  strategy = "oauth2"
  options {
    client_id = auth0_client.comp_app_client.client_id
    client_secret = auth0_client.comp_app_client.client_secret
    authorization_endpoint = "${aws_apigatewayv2_stage.default.invoke_url}authorize"
    token_endpoint = "${aws_apigatewayv2_stage.default.invoke_url}token"
    scopes = ["openid"]
    scripts = {
